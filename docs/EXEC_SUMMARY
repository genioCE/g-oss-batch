...
- **Module:** g-oss-batch (Batch/ELT orchestration)
- **Purpose:** Jobs-as-code with Airflow scheduling, dbt SQL models, and Great Expectations DQ gates.
- **Primary components:** Apache Airflow, dbt Core (Trino), Great Expectations
- **Replaces / reduces:** Control-M/Autosys scheduling; Informatica GUI ETL mappings
- **Integrates with:** g-oss-core, g-oss-cdc, g-oss-governance, g-oss-observability, g-oss-security, g-oss-bi
...
```
[Iceberg tables] ← dbt models (staging→gold) ← Airflow DAGs → GE checkpoints
```

This repository delivers the **g-oss-batch (Batch/ELT orchestration)** segment of that picture.

---

## 4) Common oil & gas use cases (examples)

- Daily production summaries with variance checks
- Finance nightly loads with control totals
- DQ gates on critical volumes and pressures
...
- **Pipeline success ≥ 99.5%**; **MTTR < 30m** with on-call runbooks.
- Freshness SLA tracked per DAG; alerts via Observability module.
...
1. **Pilot (0–90 days)**  
   - Stand up `g-oss-batch (Batch/ELT orchestration)` in a sandbox; connect a *low-risk* data domain.  
...
- **Orphaned ad-hoc SQL** → Consolidate in dbt; enforce reviews.
- **Hidden dependencies** → OpenLineage integration; ownership docs.
- **Manual runbooks** → Codify as DAG tasks; GitOps for configs.
...
- Informatica mappings → dbt models (SQL)
- Proprietary DQ → Great Expectations
